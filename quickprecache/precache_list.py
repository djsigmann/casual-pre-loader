...
def make_precache_list(game_path: str) -> Set[str]:
    ...
    model_list = set()
    custom_folder = Path(game_path) / "custom"
    ...
    # filter for "decompiled " and "competitive_badge.mdl"
    return {model for model in model_list if "decompiled " not in model and "competitive_badge.mdl" not in model}
