...
from casual_pre_loader.core.backup_manager import prepare_working_copy
from casual_pre_loader.core.config import config
from casual_pre_loader.core.constants import (
...
from casual_pre_loader.core.handlers.file_handler import FileHandler, copy_config_files, generate_config
from casual_pre_loader.core.handlers.paint_handler import disable_paints, enable_paints
from casual_pre_loader.core.handlers.pcf_handler import (
    check_parents,
    restore_particle_files,
    update_materials,
)
from casual_pre_loader.core.handlers.skybox_handler import handle_skybox_mods, restore_skybox_files
from casual_pre_loader.core.handlers.sound_handler import SoundHandler
from casual_pre_loader.core.operations.file_processors import (
    check_game_type,
    game_type,
    get_from_custom_dir,
    initialize_pcf,
)
from casual_pre_loader.core.operations.for_the_love_of_god_add_vmts_to_your_mods import (
    generate_missing_vmt_files,
)
from casual_pre_loader.core.operations.mdl_relocate import relocate_mdl_paths
from casual_pre_loader.core.operations.pcf_compress import remove_duplicate_elements
from casual_pre_loader.core.operations.pcf_rebuild import extract_elements, load_particle_system_map
from casual_pre_loader.core.operations.vgui_preload import patch_mainmenuoverride
from casual_pre_loader.core.quickprecache.precache_list import make_precache_list
from casual_pre_loader.core.quickprecache.quick_precache import QuickPrecache
from casual_pre_loader.core.util.file import check_writable, copy, delete, move
from casual_pre_loader.core.util.vpk import get_vpk_name
        ...
        try:
            ...
            if files_to_copy:
                ...
                if is_tf2:
                    ...
                    vpk_paths = []
                    misc_vpk = tf_path / "tf2_sound_misc_dir.vpk"
                    if misc_vpk.exists():
                        vpk_paths.append(misc_vpk)
                    vo_vpks = list(tf_path.glob("tf2_sound_vo_*_dir.vpk"))
            ...
            if is_tf2:
                ...
                QuickPrecache(tf_path.parents[0], debug=False).run(flush=True)
