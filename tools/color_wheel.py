...
def animate_color_shift(red_colors, red_target, blue_colors, blue_target, frames=120, interval=10, show_metrics=False,
                        save_video=False):
    plt.style.use('dark_background')
    ...
    shifted_red = color_shift(red_colors, red_target)
    shifted_blue = color_shift(blue_colors, blue_target)
    ...
    num_lines = len(red_colors) + len(blue_colors)
    ...
    target_segments = []
    target_colors = []
    for target_color in [red_target, blue_target]:
    ...
    def update(frame):
        ...
        for i, data in enumerate(color_data):
            # Interpolate HSV values for position
